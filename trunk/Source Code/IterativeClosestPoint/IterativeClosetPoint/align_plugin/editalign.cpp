...
bool InitFeaturePairs(std::vector<vcg::Point3f> *fixPnt, std::vector<vcg::Point3f> *movePnt)
{
...
bool EditAlignPlugin::glueByPicking()
{
	...
	InitFeaturePairs(&gluedPnt, &freePnt);
	...
	currentNode()->tr()=res;
	PrintMatrix44(res, "estimate.txt");
	return true;
} 
	...
	for(int i = 0; i < meshTree.gluedNum(); i++)
	{
		...
		{			
			for(int row = 0; row < 4; row++)
			{
				...
				{
					//transpose
					out << (meshTree.nodeList.at(i)->tr()).ElementAt(col, row);
					//out << (meshTree.nodeList.at(i)->tr()).ElementAt(row, col);
					out << ' ';
				}
				out << '\n';
			}			
