...
int surfMatching(string observed, string model, CvMat* &observedPoints, CvMat* &modelPoints)
{
	char* object_filename = new char[model.length()];
    char* scene_filename = new char[observed.length()];
	strcpy(object_filename, model.c_str());
	strcpy(scene_filename, observed.c_str());
	...
	*/
	...
	for(int i = 0, j = 0; i < (int)ptpairs.size(); i += 2, j++  )
    {
		...
		points2->data.fl[j*2] = r1->pt.x;
		points2->data.fl[j*2 + 1] = r1->pt.y;
		points1->data.fl[j*2] = r2->pt.x;
		points1->data.fl[j*2 + 1] = r2->pt.y;
	}
	observedPoints = points1;
	modelPoints = points2;
	...
	*/
    ...
    //int num = cvFindFundamentalMat(points1, points2, fundMatr, CV_FM_RANSAC , 1.0, 0.9999);
	/*
	// Fundamental Matrix : average error
    if (num == 1) {
        printf("Fundamental matrix was found\n");
			...
			for (int i = 0; i < 3; i++)
			{
				for (int j = 0; j < 3; j++)
				{
					printf("%f\t", CV_MAT_ELEM(*fundMatr, float,  i, j));
				}
				printf("\n");
			}
			...
			{
				CvMat* m2T = cvCreateMat(1, 3, CV_32FC1);
				CV_MAT_ELEM( *m2T, float, 0, 0) = points2->data.fl[i*2];
				CV_MAT_ELEM( *m2T, float, 0, 1) = points2->data.fl[i*2 + 1];
				CV_MAT_ELEM( *m2T, float, 0, 2) = 1;

				CvMat* m1 = cvCreateMat(3, 1, CV_32FC1);
				CV_MAT_ELEM( *m1, float, 0, 0) = points1->data.fl[i*2];
				CV_MAT_ELEM( *m1, float, 1, 0) = points1->data.fl[i*2 + 1];
				CV_MAT_ELEM( *m1, float, 2, 0) = 1;

				temp1 = cvCreateMat(1, 3, CV_32FC1);
				cvMatMul(m2T, fundMatr, temp1);
				//printf("Temp1 : \n");
				//printf("%f\t", CV_MAT_ELEM(*temp1, float,  0, 0));
				//printf("%f\t", CV_MAT_ELEM(*temp1, float,  0, 1));
				//printf("%f\t", CV_MAT_ELEM(*temp1, float,  0, 2));
				//printf("\n");

				temp2 = cvCreateMat(1, 1, CV_32FC1);					
				cvMatMul(temp1, m1, temp2);

				//printf("Temp2 : \n");
				//printf("%f\t", CV_MAT_ELEM(*temp2, float,  0, 0));
				//printf("\n");

				err += abs(CV_MAT_ELEM( *temp2, float, 0, 0));
				//printf("Err : %f\n", err);
			}
			err = err / (ptpairs.size() / 2);
			printf("Average error on %f pairs: %f\n", float(ptpairs.size() / 2), err);
    } else {
	...
	//now visualize the fundamental matrix by drawing all the epipolar lines
	/*
    int numOutputPoints;

    CvMat * corrLines;
    corrLines = cvCreateMat(3, ptpairs.size() / 2, CV_32F);
	...
	IplImage* epipolarImage = cvLoadImage( scene_filename, CV_LOAD_IMAGE_COLOR );
    ...
    for (int i1 = 0; i1 < ptpairs.size() / 2; i1++) {
        ...
        cvShowImage(windowName, object);
		cvWaitKey(0);

        cvLine(epipolarImage, epipolarLinePoint1, epipolarLinePoint2, CV_RGB(0, 255, 0));
	...
	cvShowImage(windowName, epipolarImage);
    cvWaitKey(0);
    cvSaveImage("d:\\epipolars.jpg", epipolarImage);
	*/
    return 0;
}
